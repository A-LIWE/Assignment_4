...
void main() {
  ...
  final session1     = ParkingSession(dummyVehicle, dummySpace, DateTime(2025, 5, 1, 8, 0));
  ...
  blocTest<ParkingSessionBloc, ParkingSessionState>(
    ...
    expect: () => [
      isA<SessionsLoading>(),
      isA<SessionsLoaded>()
        .having((s) => s.sessions, 'sessions', [session1, session2]),
    ],
  );
  ...
  blocTest<ParkingSessionBloc, ParkingSessionState>(
    ...
    expect: () => [
      isA<SessionsLoading>(),
      isA<SessionsError>()
        .having((s) => s.message, 'message', contains('Serverfel')),
    ],
  );
  ...
  blocTest<ParkingSessionBloc, ParkingSessionState>(
    ...
    expect: () => [
      ...
      isA<SessionsLoaded>()
        .having((s) => s.sessions, 'sessions', [session1]),
    ],
  );
  ...
  blocTest<ParkingSessionBloc, ParkingSessionState>(
    ...
    build: () {
      when(() => mockRepo.add(any()))
        .thenThrow(Exception('Redan aktiv'));
      return bloc;
    },
    act: (b) => b.add(StartSession(session1)),
    expect: () => [
      isA<SessionsLoading>(),
      isA<SessionsError>()
        .having((s) => s.message, 'message', contains('Redan aktiv')),
    ],
  );
  ...
  blocTest<ParkingSessionBloc, ParkingSessionState>(
    ...
    build: () {
      when(() => mockRepo.update(any(), newEndTime: any(named: 'newEndTime')))
        .thenAnswer((_) async => {});
      when(() => mockRepo.getAll())
    ...
    expect: () => [
      isA<SessionsLoading>(),
      isA<SessionsLoaded>()
        .having((s) => s.sessions, 'sessions', [session2]),
    ],
  );
  ...
  blocTest<ParkingSessionBloc, ParkingSessionState>(
    ...
    expect: () => [
      isA<SessionsLoading>(),
      isA<SessionsError>()
        .having((s) => s.message, 'message', contains('Ingen aktiv session')),
    ],
  );
}
